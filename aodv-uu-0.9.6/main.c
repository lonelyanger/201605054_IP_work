...
{
    static char *ifname = NULL;	/* Name of interface to attach to *//*ÒªÁ¬½Óµ½µÄ½Ó¿ÚµÄÃû³Æ*/
    fd_set rfds, readers;
    int n, nfds = 0, i;
    int daemonize = 0;
    struct timeval *timeout;
    struct timespec timeout_spec;
    struct sigaction sigact;										/*<signal.h>ÖÐ*/
    sigset_t mask, origmask;

    /* Remember the name of the executable... */
    progname = strrchr(argv[0], '/');                                  /*·µ»Ø×îºóÒ»´Î³öÏÖ¡®/¡¯µÄÎ»ÖÃ*/
    ...
    memset (&sigact, 0, sizeof(struct sigaction));                     /*ÎªÐÂÉêÇëµÄÄÚ´æ×ö³õÊ¼»¯¹¤×÷£¬È«ÖÃ0*/
    sigact.sa_handler = signal_handler;                                /*º¯ÊýÖØ¶¨Òå*/
    ...
    /* Parse command line: */
    while (1) {                                                                         /*°ÑÃüÁî²ÎÊý¶Á³ö²¢Ö´ÐÐÍê*/
	int opt;

	opt = getopt_long(argc, argv, "i:fjln:dghoq:r:s:uwxDLRV", longopts, 0);            /*´Ó²ÎÊýÖÐÔÚ¸ø¶¨ÖµÓòµ±ÖÐÑ°ÕÒ²Ù×÷·û*/
